...
**Функция:** `demonstrate_magic_methods()`

Демонстрируется работа:
- `__str__` — красивое отображение счета
- `__repr__` — представление для разработчиков
- `__eq__` — сравнение счетов по номеру
- `__lt__` — сортировка счетов по балансу

![6c](./img/lab01/6c.png)
*Работа магических методов: отображение, сравнение, сортировка*  

## Сценарий 7: Перевод между счетами
**Функция:** `demonstrate_transfer()`

Показывается:
- Баланс отправителя и получателя до перевода
- Выполнение перевода методом `transfer_to()`
...
## Сценарий 8: Работа TransactionValidator
**Функция:** `demonstrate_transaction_validator()`

Демонстрируется работа специализированного валидатора:
- Проверка корректного депозита
- Проверка депозита с отрицательной суммой
- Проверка снятия с недостаточным балансом
- Проверка корректного перевода

![8c](./img/lab01/8c.png)
*Работа TransactionValidator для проверки транзакций*
